...
function showNotification(options) {
  const { message, status, style, container } = options;

  switch (style) {
    case "toastr":
      if (typeof toastr !== "undefined") {
        // Use Toastr library if it's loaded
    ...
    case "div":
    default:
      // The default behavior: display message in a specific div
      if (container && $(container).length) {
        const alertClass = status ? "alert-success" : "alert-danger";
        const messageHtml = `<div class="alert ${alertClass} text-center" style="color: #000">${message}</div>`;
        $(container)
          .html(messageHtml)
          .fadeIn("fast")
          .delay(4000)
        ...
        console.error("Message container not found or specified.");
        alert(message); // Fallback to a simple alert
      }
      break;
  }
...
$(document).on("submit", ".ajax-form", function (e) {
  ...
  const notificationStyle = $form.data("notification-style") || "div"; // Default to 'div'
  const messageContainer = $form.data("message-container");
  const resetOnSuccess = $form.data("reset-on-success") !== false; // Default to true
  ...
  $.ajax({
    url: $form.attr("action"), // Get URL from the form's 'action' attribute
    type: "POST",
